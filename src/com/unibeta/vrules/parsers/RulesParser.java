...
public class RulesParser {
    ...
    private static final Pattern ifRegexPattern = Pattern.compile(
            "if\\s*\\n*\\t*\\r*\\v*\\f*\\e*\\a*\\b*\\({1}.+\\){1}",
            Pattern.DOTALL);
    ...
    public VRuleSuite parserRuleConfig(String fileName) throws Exception {
        ...
        if (ruleSuite.getGlobalConfig() != null
                && ruleSuite.getGlobalConfig().getIncludes().trim().length() > 0) {
    ...
    private void mergeOneEntityExtensionRules(ObjectEntity obj,
            VRuleSuite ruleSuite) {

        if (!CommonUtils.isNullOrEmpty(obj.getExtension())) {
            String[] extIds = obj.getExtension().split(",");

            for (String id : extIds) {
                ObjectEntity extObj = ruleSuite.getObjectEntities().get(
                        id.trim());
    ...
    private void validateReferenceObjectExist(VRuleSuite ruleSuite)
            throws Exception {

        Map<String, ObjectEntity> objectEntities = ruleSuite
                .getObjectEntities();
        ...
        for (ObjectEntity oe : objList) {
            for (Rule r : oe.getRules()) {
                ...
                if (null != binding
                        && !CommonUtils.isNullOrEmpty(binding.getWirings())) {
                ...
                for (String s : attributes) {
                    if (null != s
                            && (s.trim().contains(" ") || s.trim()
                                    .contains("-"))) {
                        throw new Exception(
                                "The attributes ["
                                        + r.getInputObjects()
                                        + "] contains invalid variable in rule[id = "
                                        + r.getId()
                                        + "] of Object[id = "
                                        + oe.getId()
                                        + "] from current input configuration definition file.");
    ...
    private void validateRuleDecisionConfig(
            Map<String, ObjectEntity> objectEntities) throws Exception {
            ...
            for (Rule r : obj.getRules()) {
                if (!CommonUtils.isNullOrEmpty(r.getIfTrue())) {
                    ...
                    if (!containsRule(r.getIfTrue(), tempRules)) {
                        throw new Exception(
                                "Decision configuration rules validation failed, referred [ifTrue="
                                        + r.getIfTrue() + "] is not valid in ["
                                        + r.getId() + "] rule of "
                                        + obj.getId() + decisionRules);
                    }
                }
                ...
                if (!CommonUtils.isNullOrEmpty(r.getIfFalse())) {
                    if (!containsRule(r.getIfFalse(), tempRules)) {
                        throw new Exception(
                                "Decision configuration rules validation failed, referred [ifFalse="
                                        + r.getIfFalse()
                                        + "] is not valid in [" + r.getId()
                                        + "] rule of " + obj.getId()
                                        + decisionRules);
                    }
                }
    ...
    private void sortRuleSuiteRules(VRuleSuite ruleSuite) throws Exception {
        ...
        Collection<ObjectEntity> entityValues = ruleSuite.getObjectEntities()
                .values();
    ...
    private void validateRulesDepends(ObjectEntity oe) throws Exception {
        ...
        for (Object o : oe.getRules()) {
            ...
            if (rule.getDepends().length() > 0) {
                ...
                String parseredStr = rule.getDepends().replace("&&", ",")
                        .replace("||", ",").replace("==", ",")
                        .replace("!", ",");

                if (parseredStr.contains("<") || parseredStr.contains("<=")
                ...
                for (String str : depends) {
                    ...
                    for (Object r : oe.getRules()) {
                        ...
                        if (trim.length() > 0 && trim.equals(i.getId())
                                && rule.getSequence() > i.getSequence()
                ...
                if (!finallFlag) {
                    throw new Exception(
                            "Rule's dependence configuration is invalid! The rule's dependence can only be inside the object scope and "
                                    ...
                                    + " The invalid rule id is "
                                    + rule.getId()
                                    + " of "
                                    + oe.getId()
                                    + " object."
                                    + "The invalid configuration is '"
                                    + rule.getDepends() + "'.");
                }
     ...
     *            IMPORTS,JAVA_BODY,CONTEXTS,DECISION_CONSTANT_DEFINITIONS
     *            ,OBJECT, if it is null, merge all element that needed.
     * @return
     * @throws Exception
     */
    private VRuleSuite mergeSpuerRulesByCondition(VRuleSuite ruleSuite,
            String currentfileName, String nodeFlag) throws Exception {

        String[] includesFiles = CommonUtils.fetchIncludesFileNames(ruleSuite
                .getGlobalConfig().getIncludes(), currentfileName);

        for (int i = 0; i < includesFiles.length; i++) {

            VRuleSuite ruleSuite2 = ConfigurationProxy.getVRuleSuiteInstance(
                    includesFiles[i], this.serializeClass);

            if (IMPORTS.equals(nodeFlag)) {
                mergeRulesImports(ruleSuite, ruleSuite2);
            } else if (JAVA_BODY.equals(nodeFlag) || JAVA.equals(nodeFlag)
                    || METHODS.equals(nodeFlag)) {
    ...
    private void mergeDecisionConstantDefinition(VRuleSuite ruleSuite,
            VRuleSuite ruleSuite2) {

        ruleSuite.getDecisionConstantDefinitions().addAll(
                ruleSuite2.getDecisionConstantDefinitions());
    ...
    private Map<String, String> convertToContextMap(
            List<VRules4jContext> contexts2) {
    ...
    private Map<String, String> convertToDecisionConstantDefinitionMap(
            List<DecisionConstantDefinition> contexts2) {
    ...
    private VRuleSuite parseDocument(String fileName) throws Exception {
        ...
        ruleSuite.setDeclaredMethods(CommonUtils
                .handleSpecialExpression(methods));
        ...
        this.mergeSpuerRulesByCondition(ruleSuite, fileName,
                DECISION_CONSTANT_DEFINITIONS);
        decisionConstantDefinitionMap = this
                .convertToDecisionConstantDefinitionMap(ruleSuite
                        .getDecisionConstantDefinitions());
    ...
    private Map<String, Ruleset> buildRulesetMap(Document doc) throws Exception {
        ...
        Map<String, Ruleset> rulesetMap = Collections
                .synchronizedMap(new HashMap<String, Ruleset>());
    ...
    private Map<String, ObjectEntity> buildObjectEntityMap(Document doc)
            throws Exception {

        NodeList nodeList = doc.getElementsByTagName(OBJECT);
        Map<String, ObjectEntity> rulesConfigMap = Collections
                .synchronizedMap(new HashMap<String, ObjectEntity>());
    ...
    private void validateCirculatoryIncludes(String fileName,
            List<String> circulatoryList) throws Exception {
        ...
        String[] includesFiles = CommonUtils.fetchIncludesFileNames(includes,
                fileName);
        ...
        for (String s : includesFiles) {
            ...
            if (!f.exists()) {
                throw new Exception(
                        "File not found! The included rule configuration ["
                                + f.getAbsolutePath() + "] is not found.");
            }

            if (!circulatoryList.contains(f.getAbsolutePath())
                    && !f.getAbsolutePath().equals(file.getAbsolutePath())) {
                this.validateCirculatoryIncludes(s, circulatoryList);
            } else {
                throw new Exception(
                        "Dead-lock including is detected! The file ["
                                + f.getAbsolutePath()
                                + "] is recursively included in file ["
    ...
    private List<DecisionConstantDefinition> initDecisionConstantDefinitions(
            Document doc) {
        ...
        NodeList nodes = doc
                .getElementsByTagName(DECISION_CONSTANT_DEFINITIONS);
    ...
    private List<VRules4jContext> initContexts(Document doc) {
        ...
        List<VRules4jContext> contexts = Collections
                .synchronizedList(new ArrayList<VRules4jContext>());
        Element element = null;
        ...
        for (int i = 0; i < contextList.getLength(); i++) {
            ...
            if (null != name && null != className && name.trim().length() > 0
    ...
    private ObjectEntity generateObjectEntity(Element element) throws Exception {
        ...
        long sequence = Long.valueOf(
                null == seqStr || "".equals(seqStr) ? 0 : Long.valueOf(seqStr))
                .longValue();
        ...
        entity.setClassName(XmlUtils.getNodeValueByTargetName(element,
                CLASS_NAME).trim());
        ...
        entity.setNill((nill == null) || nill.equalsIgnoreCase(BOOL_FALSE) ? false
                : true);
    ...
    private void handleObjectEntityXmlElementNames(ObjectEntity entity)
            throws Exception {
        ...
        if (null == entity.getName() || entity.getName().length() <= 0) {
            ...
            String entityXmlName = VRulesCommonUtils
                    .distillXmlElementWrapperName(annotations, entity.getName());
            if (null == entityXmlName) {
                entityXmlName = VRulesCommonUtils
                        .generateObjectEntityXmlName(entity.getId());
            }
            entity.setName(entityXmlName);
        }

        Map<String, String> map = VRulesCommonUtils
    ...
    private List generateRules(Element element) throws Exception {
        ...
        Element rulesNode = XmlUtils
                .getFirstElementByTargetName(element, RULES);
        if (null == rulesNode) {
            return list;
        }
        ...
        for (int i = 0; i < ruleNodeList.getLength(); i++) {
            ...
            if (!list.contains(rule)) {
                ...
                throw new Exception("vRules configuratuion error: The id ["
                        + rule.getId() + "] is duplicated in the same object.");
            }
        }
    ...
    private Rule generateRule(Element ruleElement) throws Exception {
        ...
        String brakpointStr = ruleElement.getAttribute(BRAKPOINT).toUpperCase()
                .trim();
        ...
        rule.setDataType(XmlUtils.getNodeValueByTargetName(ruleElement,
                DATA_TYPE));
        String attributes = XmlUtils.getNodeValueByTargetName(ruleElement,
                ATTRIBUTES).trim();

        if (attributes.equals("")) {
            attributes = XmlUtils.getNodeValueByTargetName(ruleElement,
                    INPUT_OBJECTS).trim();
        }
        rule.setInputObjects(attributes);

        String boolParam = XmlUtils.getNodeValueByTargetName(ruleElement,
                OUTPUT_BOOL);
        if (CommonUtils.isNullOrEmpty(boolParam)) {
            boolParam = XmlUtils.getNodeValueByTargetName(ruleElement,
                    BOOL_PARAM);
        }
        rule.setOutputBool(boolParam.trim());

        /*
         * Element e = XmlUtils .getFirstElementByTargetName(ruleElement,
         * PREDICATE); String value = e.getFirstChild().getTextContent();
         * rule.setPredicate(value);
         */

        String assertion = XmlUtils.getNodeValueByTargetName(ruleElement,
                ASSERT).trim();
        if (CommonUtils.isNullOrEmpty(assertion)) {
            assertion = XmlUtils.getNodeValueByTargetName(ruleElement,
                    PREDICATE).trim();
        }
        ...
        if (rule.getId().length() == 0) {
            logger.warn("Parsing the rules configuration file failed. The arrtibute 'id' is mandatory for rule element!");
            return null;
        }
        ...
        Element bindNode = XmlUtils.getFirstElementByTargetName(ruleElement,
                "binding");
    ...
    private void setErrorObject(Element ruleElement, Rule rule)
            throws Exception {

        String errorMessage = generateErrorMessage(ruleElement, rule);
        String sequenceId = ErrorObjectPoolManager.generateSequenceId()
                .toString();

        errorMessage = sequenceId + VRulesConstants.$_ERROR_MESSAGE_FLAG
                + errorMessage;
        ...
        if (null != serializeClass) {
            ...
            if (null != decisionObjectTarget) {
                ...
                if (RULE.equals(decisionObjectTarget.getParentNode()
                        .getNodeName())) {
                    if (decisionObjectTarget.getFirstChild() != null) {
                        registeredNode = decisionObjectTarget.getFirstChild()
                                .getNextSibling();
                    }
                } else {
            ...
            if (registeredNode != null) {
                unmarshalToObject = ObjectSerializer.unmarshalToObject(
                        registeredNode, serializeClass);
                processPredefinedDecisionConstant(unmarshalToObject,
                        decisionConstantDefinitionMap);
                ErrorObjectPoolManager.addError(sequenceId, unmarshalToObject);
            } else {
                logger.warn("Decision object was not defined for rule[id="
                        + rule.getId()
    ...
    private Element getDecisionElement(Element ruleElement) {
        ...
        Element decisionObjectTarget = XmlUtils.getFirstElementByTargetName(
                ruleElement, ERROR_OBJECT);
        if (null == decisionObjectTarget) {
            decisionObjectTarget = XmlUtils.getFirstElementByTargetName(
                    ruleElement, DECISION_OBJECT);
        }
        if (null == decisionObjectTarget) {
            decisionObjectTarget = XmlUtils.getFirstElementByTargetName(
                    ruleElement, DECISION);
        }
        return decisionObjectTarget;
    }

    private void processPredefinedDecisionConstant(Object unmarshalToObject,
            Map<String, String> decisionConstantDefinitionMap2)
            throws Exception {
        ...
        for (Method m : methods) {
            if (m.getReturnType().equals(String.class)) {
                ...
                if (null != value && value.trim().length() > 0) {
                    definedConstantValue = decisionConstantDefinitionMap2
                            .get(value.trim());
                }

                if (null != definedConstantValue) {
                    String setMethodName = "s" + methodName.substring(1);
                    Method method = class1.getMethod(setMethodName,
                            String.class);
                ...
                if (!returnType.startsWith("java.")
                        && !m.getGenericReturnType().getClass().getName()
                                .startsWith("javax.")
                        && !m.getGenericReturnType().getClass().getName()
                                .startsWith("sun.")
                    ...
                    processPredefinedDecisionConstant(v,
                            decisionConstantDefinitionMap2);
    ...
    private void handlePredicateExpression(Rule rule) throws Exception {
        ...
        boolean hasIfExpression = ifRegexPattern.matcher(rule.getPredicate())
                .find();
        ...
        if ((null == rule.getOutputBool() || rule.getOutputBool().length() == 0)) {
            ...
            if (hasIfExpression) {
                throw new Exception(
                        "Syntax parsing fatal error in rule[id="
                                + rule.getId()
            ...
            /*
             * if not using the outBool parameter in the predicate, consider it
             * is a bool expression but not a statement.
             */
    ...
    private void constructPlainAssertation(Rule rule) {
        ...
        if (!rule.getPredicate().trim().endsWith(";")) {
            ...
            rule.setOutputBool("$");
            String predicate = "if(" + rule.getPredicate()
                    + "){$ = true;}else{$ = false;}";
            rule.setPredicate(predicate);
        }
    }

    private String generateErrorMessage(Element ruleElement, Rule rule)
            throws Exception {

        Element errorElement = XmlUtils.getFirstElementByTargetName(
                ruleElement, ERROR_MESSAGE);

        if (null == errorElement) {
            return "No errorMessage was defined for rule[id=" + rule.getId()
                    + "]";
        }

        if ((globalConfig.isReturnErrorId()
                || this.isTrue(errorElement.getAttribute(RETURN_ERROR_ID)) || this
                    .isTrue(errorElement.getAttribute(RETURN_ID)))) {
            ...
            if (null == errorId || errorId.length() == 0) {
                ...
                throw new Exception(
                        "Configuration rules fatal error, the errorId can not be null or empty for [rule id ="
                                + rule.getId()
                                + "], if the 'returnErrorId' element has been set 'true'!");
            }
            return errorId.trim();
            // rule.setErrorMessage(errorId);

        } else {
            String errorMsg = XmlUtils.getNodeValueByTargetName(ruleElement,
                    ERROR_MESSAGE).trim();
    ...
    public GlobalConfig getGlobalConfig(InputStream inputStream)
            throws Exception {
    ...
    private GlobalConfig initGlobalConfig(Document doc) {
        ...
        String returnId = XmlUtils.getNodeValueByTargetName(element,
                RETURN_ERROR_ID).trim();
        if (CommonUtils.isNullOrEmpty(returnId)) {
            returnId = XmlUtils.getNodeValueByTargetName(element, RETURN_ID)
                    .trim();
        }
        String returnErrorValue = XmlUtils.getNodeValueByTargetName(element,
                DISPLAY_ERROR_VALUE).trim();
        String includes = XmlUtils.getNodeValueByTargetName(element, INCLUDES)
                .trim();
        String toggleBreakpointStr = XmlUtils.getNodeValueByTargetName(element,
                TOGGLE_BREAKPOINT).trim();
        String decisionMode = XmlUtils.getNodeValueByTargetName(element,
                DECISION_MODE).trim();
        String assertionLogic = XmlUtils.getNodeValueByTargetName(element,
                ASSERTION).trim();
        String newInstanceIfNull = XmlUtils.getNodeValueByTargetName(element,
                ENABLE_BINDING).trim();
        String mergeExtensions = XmlUtils.getNodeValueByTargetName(element,
                MERGE_EXTENSIONS).trim();

        globalConfig.setReturnErrorId(isTrue(returnId) ? true : false);
        globalConfig.setReturnErrorValue(isTrue(returnErrorValue) ? true
                : false);
        globalConfig.setIncludes(includes);
        globalConfig.setToggleBreakpoint(isTrue(toggleBreakpointStr) ? true
                : false);
    ...
    private boolean isTrue(String decision) {
        ...
        return BOOL_TRUE.equalsIgnoreCase(decision.trim())
                || BOOL_ON.equalsIgnoreCase(decision.trim());
    }
