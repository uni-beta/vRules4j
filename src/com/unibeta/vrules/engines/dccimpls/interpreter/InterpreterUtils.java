...
public class InterpreterUtils {
    ...
    private static Pattern evalPattern = Pattern.compile(REGEX_EVAL_EXPRESSION,
            Pattern.DOTALL);
    private static Boolean $hasBsh = null;

    /**
     * Gets the value if the given className is equal or assignable with the
     * value instance from map.
     * 
     * @param map
     * @param className
     * @return
     * @throws Exception
     */
    public static Object getValueFromCollectionByClassName(
            Collection<Object> c, String className) throws Exception {
    ...
    public static String bshEval(Interpreter bsh, String msg) {
        ...
        if (!hasBsh() || null == bsh || null == msg
                || !evalPattern.matcher(msg).find()) {
            return msg;
        }
        ...
        for (String expr : exprs) {
            ...
            if (evalPattern.matcher(expr).find()) {
                ...
                try {
                    ...
                    String errormsg = "'evaluate #{" + express
                            + "} error, caused by " + e.getMessage()+"'";
                    log.error(errormsg, e);
                    results.add(errormsg);
                }
            }
            format.append(expr.replaceAll(REGEX_EVAL_EXPRESSION, STRING_FORMAT).replace("%", "%%").replace(STRING_FORMAT, "%s"));
        }
    ...
    public static String plainEval(Object o, String msg) {
        ...
        if (evalPattern.matcher(msg).find()) {
            ...
            result = msg.replace(prefix + express + "}",
    ...
    public static void bshSet(Interpreter bsh, String name, Object obj) {
        ...
        try {
            ...
            log.error(
                    "BeanShell set variable error caused by " + e.getMessage(),
                    e);
        }
    }
    ...
    public static void bshUnset(Interpreter bsh, String name) {
        ...
        try {
            ...
            log.error(
                    "BeanShell unset variable error caused by "
                            + e.getMessage(), e);
        }
    }
    ...
    public static boolean isBooleanType(String className) {
        ...
        if (boolean.class.getCanonicalName().equals(className)
                || Boolean.class.getCanonicalName().equals(className)) {
            return true;
        } else {
            return false;
        }
