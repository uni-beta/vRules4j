...
/**
 * CommonSyntaxs is a common validation tool delegated by vRules4j engine.
 * All of them can be used in rule's definition expression directly.
 * 
 * For Example:
 * println("this is a println for var1 ",var1,", print other var2: ", var2);
 * 
 * @author jordan.xue
 */
public class CommonSyntaxs {
    ...
    public static boolean dateFormat(String regex, String value) {
        ...
        regex = regex.replace("mm", "MIN").toUpperCase()
                .replace("YYYY", REGEX_YEAR).replace("YY", REGEX_YEAR)
                .replace("MM", REGEX_MONTH).replace("DD", REGEX_DAY)
                .replace("HH", REGEX_HOUR).replace("MIN", REGEX_MININUTE)
                .replace("SS", REGEX_SECOND);
    ...
    public static boolean decimal(String digital, String decimal, String value) {
        ...
        if (null != values && values.length == 1) {
            ...
            if (decimalStr.length() > decimalLength
                    || digitStr.length() > digitalLength - decimalLength) {
                return false;
            }
        } else {
            return false;
