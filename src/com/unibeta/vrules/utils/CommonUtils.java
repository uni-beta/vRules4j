...
public class CommonUtils {
     ...
     * 
     * @param fileName
     * @return null if the fileNanme is of a directory. otherwise return the
     *         file name.
     */
    ...
    /**
     * Try to find specified file called <code>findFileName</code> under the
     * root file <code>rootFileName</code>.
     * 
     * @param rootFileName
     *            - the root file name.
     ...
     * @param ignoreRegex
     * @param needNested
     * @return - the found file's name if find successfully; otherwise return
     *         null.
     */
    public static List<String> searchFilesUnder(String rootFileName,
            String targetFileNameRegex, String ignoreRegex, boolean needNested) {

        List<String> list = new ArrayList<String>();

        if (CommonUtils.isNullOrEmpty(rootFileName)
                || CommonUtils.isNullOrEmpty(targetFileNameRegex)) {
            return list;
        }
            ...
            for (int i = 0; i < subFiles.length; i++) {
                if (null != ignorePattern) {
                    if (needNested && subFiles[i].isDirectory()) {
                        list.addAll(searchFilesUnder(subFiles[i].getPath(),
                                targetFileNameRegex, ignoreRegex, needNested));
                    } else if (targetPattern.matcher(subFiles[i].getName())
                            .find()
                            && !ignorePattern.matcher(subFiles[i].getName())
                                    .find()) {
     ...
     */
    public static List<String> searchFilesAbove(String fileName,
            String targetFileNameRegex, String ignoreRegex, int superIndex) {
        ...
        do {
            ...
            if (null == tp) {
                ...
                for (int i = 0; i < files.length; i++) {
                    list.addAll(searchFilesUnder(files[i].getPath(),
                            targetFileNameRegex, ignoreRegex, false));
     ...
     * @return
     */
    public static String findSuperParentFileName(String currentFileName,
            int superIndex) {
    ...
    /**
     * Digest the inclues file name via given current file name. '../' stands
     * for the previous patch; './' stands for current sub folder name.
     * 
     * @param includes
     ...
     */
    public static String[] fetchIncludesFileNames(String includes,
    ...
    /**
     * Judges whether the input object is null or empty. Return true, if the obj
     * is null or obj.toString().trim().length()==0;
     */
