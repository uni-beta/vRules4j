...
public class URLConfiguration {
        ...
        for (int i = 0; filePathNames != null && i < filePathNames.length; i++) {
            ...
            if (filePathNames[i].length() > 0) {
                ...
                try {
                    ...
                    log.error("generateUrlsByClasspath from " + classpath
                            + "failed, caused by ", e);
                }
            }
        }
    ...
    private static String generateClassPath(String realPath, int searchType) {
        ...
        case SEARCH_BY_WEB_CONTEXT_PATH: {
            ...
            webinfFloderList = filterWebInfWithJars(CommonUtils
                    .searchFilesUnder(realPath, WEB_INF_REGEX, CLASSES, true));
        ...
        case SEARCH_BY_CLASSLOADER: {
            ...
            for (int i = 0; i <= 5; i++) {
                ...
                if (i == 0) {
                    String root = CommonUtils.findSuperParentFileName(realPath,
                            i + 1);
                    webinfFloderList = filterWebInfWithJars(CommonUtils
                            .searchFilesUnder(root, WEB_INF_REGEX, CLASSES,
                                    true));
                } else {
                    String root = CommonUtils.findSuperParentFileName(realPath,
                            i - 1);
        ...
        for (String webinf : webinfFloderList) {
            ...
            libraries = findAllLibraries(webinf + File.separator + LIB
                    + File.separator);
            sb.append(classes + CLASSPATH_SEPARATOR + libraries
                    + CLASSPATH_SEPARATOR);
        }
    ...
    private static List<String> filterWebInfWithJars(List<String> findFilesUnder) {
        ...
        for (String path : findFilesUnder) {
            ...
            if (libContainsJars(findAllLibraries(path + File.separator + LIB
                    + File.separator))) {
                list.add(path);
            }
        }
    ...
    private static String findAllLibraries(String fileName) {
        ...
        for (int i = 0; files != null && i < files.length; i++) {
            ...
            if (f.isFile()) {
                if (f != null && null != f.getName()
                        && (f.getName().toLowerCase().endsWith(JAR_SUFFIX))) {
    ...
    public static void initClasspathURLs() {
        ...
        if (CommonUtils.isNullOrEmpty(URLConfiguration.getClasspath())) {
            ...
            URLConfiguration.setClasspath(classpath + File.pathSeparator
                    + parentClassLoaderLibs + File.pathSeparator
            ...
            URLConfiguration
                    .setClasspathURLs(generateUrlsByClasspath(URLConfiguration
                            .getClasspath()));
        }
    }
    ...
    private static void resolveClasspath() {
        ...
        String[] paths = URLConfiguration.getClasspath().split(
                File.pathSeparator);
        StringBuffer sb = new StringBuffer();
    ...
    private static void setExtraClasspath() {
        ...
        if (!CommonUtils.isNullOrEmpty(paths)) {
            ...
            for (String s : ps) {
                URLConfiguration.setClasspath(URLConfiguration.getClasspath()
                        + File.pathSeparator + findAllLibraries(s)
    ...
    public static String getLocalClassPaths(){
        ...
        if (!CommonUtils.isNullOrEmpty(URLConfiguration.getRealPath())) {
            classpath = generateClassPath(URLConfiguration.getRealPath(),
                    SEARCH_BY_WEB_CONTEXT_PATH);
        } else {
